...
These functions take an array, shuffle it, then check that no more than *n* consecutive elements are the same.
...
- ```n``` - (integer) the maximum number of identical consecutive elements permitted
...
- This isn't fast!
  - Using Python the function is rapid when checking for 3+ consecutive elements in an array of ~100 elements. Larger arrays will take longer to check.
